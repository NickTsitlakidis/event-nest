...
export class MySQLEventStore extends AbstractEventStore {
    ...
    constructor(
        ...
        super(eventEmitter);
        this._logger = new Logger(MySQLEventStore.name);
        this._databaseConfiguration = new DatabaseConfiguration(databaseName, aggregatesTableName, eventsTableName);
    }

    /**
     * @deprecated Use {@link databaseConfiguration} instead. It will be removed in version 5.x
     */
    get aggregatesTableName(): string {
        return this.databaseConfiguration.aggregatesTable;
    }
    ...
    get databaseName(): string {
        return this.databaseConfiguration.database;
    }
    ...
    async findByAggregateRootId<T extends AggregateRoot>(
        ...
        const rows = await this._knexConnection<EventRow>(this._databaseConfiguration.databaseAwareEventsTable)
            .select("*")
            .where({
        ...
        try {
            await this._knexConnection.transaction(async (trx) => {
                ...
                if (isNil(foundAggregate)) {
                    ...
                    await trx(this._databaseConfiguration.databaseAwareAggregatesTable).insert({
                        id: aggregate.id,
                ...
                await trx<EventRow>(this._databaseConfiguration.databaseAwareEventsTable).insert(mapped);
                await trx<AggregateRootRow>(this._databaseConfiguration.databaseAwareAggregatesTable)
